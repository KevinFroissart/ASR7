...
void draw_screen_worker(int &slices)
{
	while (1)
	{
		mtx.lock();
...
{
	std::vector<std::thread> listeThread;
	int i;
	int slices = 0;
	for (i = 0; i < number_of_threads; i++)
	{
		listeThread.push_back(std::thread(draw_screen_worker, std::ref(slices)));
	}
	for (std::thread &t : listeThread)
	{
		t.join();
	}
	return slices;
