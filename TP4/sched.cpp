...
void change_ordonnancement(int policy, int priorite) {
  printf("Change ordo ");
  pthread_t target_thread = pthread_self();

  int old_policy;
  struct sched_param param;

  int x =  pthread_getschedparam(target_thread, &old_policy, &param);
  handle_error_en(x, "pthread_getschedparam");

  param.sched_priority = priorite;
  x =  pthread_setschedparam(target_thread, policy, &param);
  handle_error_en(x, "pthread_setschedparam");	
...
int main(int argc, char* argv[]) {
  ...
  tab.push_back(std::thread(fonction, tab.size(), nbtours, SCHED_FIFO, 2));
  tab.push_back(std::thread(fonction, tab.size(), nbtours, SCHED_FIFO, 2));
